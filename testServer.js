...
app.get('/', function (req, res) {
  getAllPosts().then(function (allPosts) {
    ...
    res.send(
      mustache.render(homepageTemplate, { postListHTML: renderPost(allPosts) })
    ) // Creates the template for Homepage.html
  })
...
app.get('/users/:slug', function (req, res) {
  ...
  console
    .log('wut is slug', res.params.slug)
    .then(function (user) {
...
function getSingleUser (slug) {
  ...
  return db
    .raw('SELECT * FROM "Users" WHERE "slug" = \'?\'', [slug])
